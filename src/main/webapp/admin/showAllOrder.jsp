...
<script type="text/javascript">
	function sendOrder(id) {
		...
		location.href = "${pageContext.request.contextPath}/admin/changeOrder.action?oid="
				+ id;
	}
	$(
			function() {
				$("#search")
						.click(
								function() {
									...
									location.href = "${pageContext.request.contextPath}/admin/getAllOrder.action?username="
											+ username
											+ "&status="
											+ status
											+ "&num=" + Math.random();
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<div class="row">
						...
						<div class="col-xs-5 col-sm-5 col-md-5 col-lg-5">
							<div class="form-group form-inline">
								<span>订单状态</span> <select name="status" class="form-control">
					...
					<div style="height: 400px;overflow: scroll;">
									...
									<td>
										...
										<c:if test="${order.status eq 2}">
											<button type="button" class="btn btn-danger btn-sm"
												onclick="sendOrder('${order.id}')">发货</button>
										</c:if>
									</td>
								</tr>
							</c:forEach>
						...
						<ul class="pagination">
							...
							<c:if test="${pageBean.pageNum>1 }">
								<li><a
									href="${pageContext.request.contextPath }/admin/getAllOrder.action?pageNum=${pageBean.pageNum-1}&pageSize=${pageBean.pageSize}&username=${username}&status=${status}"
									aria-label="Previous"><span aria-hidden="true">«</span></a></li>
							</c:if>

							<c:forEach var="pn" begin="${pageBean.startPage }"
								end="${pageBean.endPage }" step="1">
								<c:if test="${pn==pageBean.pageNum }">
									<li class="active"><a href="#">${pn }<span
											class="sr-only">(current)</span></a></li>
								</c:if>
								<c:if test="${pn!=pageBean.pageNum }">
									<li><a
										href="${pageContext.request.contextPath }/admin/getAllOrder.action?pageNum=${pn }&pageSize=${pageBean.pageSize}&username=${username}&status=${status}">${pn }</a></li>
								</c:if>
							</c:forEach>
