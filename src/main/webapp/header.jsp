...
<script type="text/javascript">
	$(document).ready(function(){
		$.ajax({
			url:"${pageContext.request.contextPath}/goodstype/getgoodstypelist.action",
			type:"GET",
			success:function(data){
				for(var i in data){
					var a = $("<a href='${pageContext.request.contextPath}/goods/getgoodslistbytypeid.action?typeId="+data[i].id+"'>"+data[i].name+"</a>");
					$("#goodsType").append(a);
    	...
    	<div id="topdiv">
            ...
            <span style="float:right">
       			...
       			<c:if test="${not empty user}">
       				<a href="/self_center.jsp" id="a_top">${user.username}</a>
       				<li>|</li>
...
<div id="second">
       ...
       <form  action="${pageContext.request.contextPath }/goods/searchgoods.action" method="get" class="form-inline pull-right" style="margin-top: 40px;margin-right: 10px;">
		
		  <div class="form-group">
		  <input type="hidden" name="method" value="search" />
		    <input type="text" class="form-control" style="width: 400px"  placeholder="搜索一下好东西..." name="search">
		  </div>
